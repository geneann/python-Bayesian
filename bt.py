...
def naive_bayes_classify(info, sentence):
    ...
    # list of probs for sentiments given tokes
    probssgt = []
    for i in range(5):
        # list of probs for token given sentiments
        probstgs = []
        # computes a prior
        prior = info.sentiment_counts[i]/info.total_examples
        for t in tokened_sentence:
            try:
                # computes prob for a given token
                prob = info.word_counts[i][t]
                if prob == 0:
                    probstgs.append(OUT_OF_VOCAB_PROB)
                else:
                    probstgs.append(info.word_counts[i][t]/info.total_words[i])
            except KeyError:
                probstgs.append(OUT_OF_VOCAB_PROB)

        logprobs = math.log(prior)

        for j in probstgs:
            logprobs += math.log(j)

        probssgt.append(logprobs)
    a = probssgt[0]
    count = -1
    index_from_count = 0
    # finds the biggest prob and its index 
    for l in probssgt:
        count += 1
        if l >= a:
            a = l
            index_from_count = count     

    return index_from_count, a # best class, log probability
...
def markov_model_classify(info, sentence):
    # tokenizes the given sentence    
    tokened_sent = tokenize(sentence)
    # list of probs for sentiments given bigram
    probssgb = []
    for i in range(5):
        # computes a prior
        prior = info.sentiment_counts[i]/info.total_examples
        # list of probs for bigram given sentiments        
        probsbgs = []
        # first word evaluation as in Bayes
        try:
            prob = info.word_counts[i][tokened_sent[0]]
            if prob == 0:
                probsbgs.append(OUT_OF_VOCAB_PROB)
            else:
                probsbgs.append(info.word_counts[i][tokened_sent[0]]/info.total_words[i])
        except KeyError:
                probsbgs.append(OUT_OF_VOCAB_PROB)
        prev_t = tokened_sent[0]
        # loop for the rest of the words
        for t in range(1, len(tokened_sent)):
             try:
                # bigram b
                b = (prev_t, tokened_sent[t])
                prob = info.bigram_counts[i][b]
                if prob == 0:
                    probsbgs.append(OUT_OF_VOCAB_PROB)
                else:
                    probsbgs.append(info.bigram_counts[i][b]/info.bigram_denoms[i][prev_t])
             except KeyError:
                probsbgs.append(OUT_OF_VOCAB_PROB)

             prev_t = tokened_sent[t]

        logpr = math.log(prior)

        for j in probsbgs:
            logpr += math.log(j)

        probssgb.append(logpr)
    a = probssgb[0]
    count = -1
    index_from_count = 0
    for l in probssgb:
